...
def delete_person(person_id):
    ...
    indices_to_remove = [i for i, pid in enumerate(face_database["person_ids"]) if pid == person_id]
    
    if not indices_to_remove:
        return False
        
    # Remove from database
    name = face_database["names"][person_id]
    face_count = len(indices_to_remove)
    
    # Delete person info
    del face_database["names"][person_id]
    del face_database["rooms"][person_id]
    del face_database["timestamps"][person_id]
    del face_database["embedding_count"][person_id]
    
    # If using FAISS, rebuild the index without these embeddings
    if FAISS_AVAILABLE and face_database["embeddings"]:
        # Create new lists without the deleted person
        new_embeddings = []
        new_person_ids = []
        
        for i, (emb, pid) in enumerate(zip(face_database["embeddings"], face_database["person_ids"])):
            if pid != person_id:
                new_embeddings.append(emb)
                new_person_ids.append(pid)
                
        # Update database
        face_database["embeddings"] = new_embeddings
        face_database["person_ids"] = new_person_ids
        
        # Rebuild index
        if new_embeddings:
            dimension = len(new_embeddings[0])
            face_database["index"] = initialize_index(dimension)
            face_database["index"].add(np.array(new_embeddings).astype('float32'))
        else:
            face_database["index"] = None
    
    print(f"Deleted {name} with {face_count} faces from database")
    return True
...
def get_all_people():
    ...
    Returns:
        list of dicts: List of people with their information
    """
    people = []
    
    for person_id in face_database["names"]:
        people.append({
            "id": person_id,
            "name": face_database["names"][person_id],
            "room": face_database["rooms"][person_id],
            "face_count": face_database["embedding_count"][person_id],
            "last_seen": face_database["timestamps"].get(person_id, 0)
        })
